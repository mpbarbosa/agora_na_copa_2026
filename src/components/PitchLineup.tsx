...
import { Match, Player, Position } from "../types";
...
export const PitchLineup: React.FC<PitchLineupProps> = ({ match }) => {
  ...
  const team = activeTeam === "A" ? match.teamA : match.teamB;
    ...
    <div className="w-full flex flex-col space-y-4" id="pitch-container">
      ...
      <div className="flex border-b border-white/10" id="pitch-team-tabs">
        <button
          ...
          className={`flex-1 py-3 text-center uppercase tracking-widest font-anton transition-all ${
            activeTeam === "A"
              ? "text-[#ffd700] border-b-2 border-[#ffd700] bg-white/5"
              : "text-white/80 hover:text-white"
          }`}
        >
          {match.teamA.name} ({match.teamA.lineup.length} Jogadores)
        </button>
        <button
          id="btn-team-b"
          onClick={() => {
        ...
        >
          {match.teamB.name} ({match.teamB.lineup.length} Jogadores)
        </button>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col items-center justify-center p-2 bg-gradient-to-b from-[#051a10] to-[#010905] rounded-xl border border-white/5 relative overflow-hidden" id="pitch-canvas-wrapper">
          ...
          <div className="w-full aspect-[3/4] md:aspect-[4/5] bg-[#0c2e1b] rounded-lg relative overflow-hidden p-6 border-2 border-white/20 select-none shadow-inner" id="soccer-pitch">
            ...
            {team.lineup.map((player) => {
              ...
              // If it's Team B, we mirror the layout vertically for premium head-to-head depiction
              const topY = activeTeam === "A" ? player.y : 100 - player.y;
                ...
                <div>
                  ...
                  <div className="mt-4 p-3 bg-white/5 rounded-lg border border-white/5" id="pundit-quote">
                    <p className="text-sm text-white/85 italic font-archivo leading-6">
                      "Para esta partida contra a {activeTeam === "A" ? match.teamB.name : match.teamA.name}, {selectedPlayer.name} desempenha papel fundamental no bloco tático {selectedPlayer.position === Position.FW ? "ofensivo para que romper a última linha adversária" : "sustentando a posse ou as saídas rápidas"}. "
                    </p>
                  </div>
                </div>
