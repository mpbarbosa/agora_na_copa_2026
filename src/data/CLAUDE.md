...
| `goalTimeline.json` | Generated | Per-finished-match list of GOAL minutes (elapsed clock; stoppage folded in, e.g. 45'+5' → 50), keyed by match id (value = sorted `number[]`). Generated by `scripts/build-goal-timeline.py` from the production `/api/match-overlays` incidents (raw JSON); every `type: "GOAL"` counts (own goals/penalties included), so per-match counts reconcile with the scoreline. Consumed by `src/dashboardStats.ts` (`goalsByMinute`) for the Dashboard "Gols por minuto" scatter plot. Re-run the script to refresh after new matches finish; do not hand-edit. |
