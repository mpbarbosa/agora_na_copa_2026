...
| `referee-card.spec.ts` | Referee card (Ao Vivo): stubs a FIFA-assigned referee onto a live match overlay, clicks the scoreboard `#match-referee-chip` → opens `#referee-card`, expands the Instagram highlight and asserts the outbound link (`target=_blank`, `rel=noopener`), then closes. The stubbed name carries an extra surname to exercise the tolerant name-match; offline-safe (routes the IG embed to empty) | 2.7 KB |
| `reddit-posts.spec.ts` | "Repercussão no Reddit" feed on Redes Sociais: `/api/reddit` returns the resilience shape (`source`/`note`/`updatedAt`/`posts`) with curated posts; the feed section renders ≥1 outbound link card (`#reddit-post-card-*`, `href` to reddit.com, `target=_blank`, `rel=noopener`); dark-theme without console errors. In CI there are no `REDDIT_*` vars, so the endpoint returns the curated seed deterministically (offline-safe) | 2.3 KB |
...
| Social Medias ("Redes Sociais" tab) | `social-medias.spec.ts`, `instagram-highlights.spec.ts`, `reddit-posts.spec.ts` |
