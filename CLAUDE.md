...
- API endpoints: `/api/broadcast-guide`, `/api/match-states`, `/api/match-overlays`, `/api/team-lineups`, `/api/tournament-leaders`, `/api/player-stats/:teamCode/:playerName`, `/api/player-incidents/:teamCode/:playerName`, `/api/team-view/:teamCode`, `/api/country-info/:code`, `/api/google-trends`, `/api/match-weather`, `/api/reddit`, `/api/questions`, `/api/predict`, `/api/qualification-odds/:teamCode`, `/api/geo`, `/api/fifa-proxy/*`, `/api/fifa-sync-status`, `/api/health`.
- **`/api/broadcast-guide?country=XX`** and **`/api/match-overlays?country=XX`** are country-aware: FIFA's "Onde Assistir / Dónde Ver" watch feed is **per-country**, so the `country` param (ISO-2, default `BR`) selects a nation's broadcasters. The curated local `match.broadcasters` fallback is Brazil-specific, so it is only used when `country === "BR"`; other countries with no FIFA data get an honest empty guide (never Brazilian channels). The guide is cached per `country:language`. The picker (`BroadcastCountrySelect`, offering the 20 live-verified countries in `src/data/broadcastCountries.ts`) sits in the Ao Vivo broadcast guide; its initial country comes from `/api/geo`, else the locale default (`pt`→BR, `es`→MX).
- **`/api/geo`** resolves the caller's country (ISO-2) from `req.ip` via the local GeoLite2-Country mmdb (`geo-core.ts` + the `maxmind` reader; `GEO_DB` overrides the path, default `/var/lib/GeoIP/GeoLite2-Country.mmdb`). Not FIFA-sourced, no resilience shape; returns `{ country: null }` whenever the db is absent (preview builds, CI/Docker) or the IP doesn't resolve — the client then falls back to its locale default. No user IP leaves the host.
