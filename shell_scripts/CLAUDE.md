...
| `08_setup_monitoring.sh` | Once (idempotent, re-runnable) | Enables timed nginx access logging (`agora_timed` log format with `rt=` and `urt=` fields); safe to run after certbot — injects surgically rather than overwriting. Also installs the `agora-traffic-report` systemd timer that hourly `git pull`s + runs `scripts/traffic-report.sh` (so the box always reports from the current script, not a stale checkout); skip with `SKIP_REPORT_TIMER=1`. Fully idempotent — safe to re-run |
...
| `10_go_live.sh` | Every prod go-live | **Build-free** rollout: pulls the prebuilt payload from the mpbarbosa.com checkout and runs `06_redeploy.sh` with `AGORA_STAGING_DIR` pinned to it — NO vite/esbuild on the box. **Version-guarded**: rolls out only when the live `/api/health` version is behind the payload (else skips); fail-open, override with `AGORA_FORCE_GO_LIVE=1`. Run this on prod instead of `npm run deploy` (also `npm run go-live`). Needs the mpbarbosa.com repo cloned on the host |
