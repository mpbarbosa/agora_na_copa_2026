...
| `sync-match-results.py` | Python 3 | Updates `status` and `score` in `src/matches.json`. Never touches lineups, broadcasters, or other curated fields. Source is selectable via `--source auto\|fifa\|production`: `auto` (default) tries the FIFA calendar API then falls back to the production site API; `production` uses `https://copa2026.mpbarbosa.com/api/match-overlays` directly (keyed by our own match ids, reachable when api.fifa.com is not). `deploy.sh` calls it with `--source production`. | Before every deploy to bake in current results. |
...
| `build-goal-timeline.py` | Python 3 | Writes `src/data/goalTimeline.json` — per-finished-match GOAL minutes (elapsed clock) from the production `/api/match-overlays` incidents (raw JSON). Powers the Dashboard "Gols por minuto" scatter. Supports `--dry-run`. | After new matches finish, to refresh the goals-by-minute chart. |
| `build-match-disciplinary.py` | Python 3 | Writes `src/data/matchDisciplinary.json` — per-team card outcome counts per finished match from `/api/match-overlays` incidents (raw JSON), for the Art. 13.2f fair-play tiebreaker. Supports `--dry-run`. | After new matches finish, to refresh fair-play points. |
